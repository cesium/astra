  ...
  return (
    <div className="flex flex-col gap-2 lg:w-[412px]">
      ...
      <SideSectionDisclosure title="Current state">
        <div className="my-2 flex w-full gap-2 items-center">
          <span className="w-1/2 sm:w-2/3 text-center font-semibold text-black/50">
            Your curricular units
          </span>
          <span className="w-1/2 sm:w-1/3 text-center font-semibold text-black/50">
            Shifts
          </span>
        </div>
        <div className="flex flex-col gap-2">
          {ucs.map((uc, index) => (
            <div key={index} className="flex w-full items-center gap-2">
              <div className="line-clamp-1 flex h-12 w-1/2 sm:w-2/3 items-center rounded-2xl bg-black/5 px-3 text-sm">
                <span className="line-clamp-1">{uc.uc}</span>
              </div>
              <div
                className={`flex h-12 w-1/2 sm:w-1/3 flex-wrap items-center justify-center gap-1 rounded-full bg-black/5`}
              >
